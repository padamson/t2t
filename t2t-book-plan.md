...
**Title:** *Trunk to Table: Building a Full-Stack Rust Application with Agentic Continuous Delivery*
...
| 03 | The Database | scaffold | `chapter-03` | PostgreSQL via Terraform, grocery_items migration, service layer, thiserror, first unit test, first cargo-mutants run | ch02 tagged | |
| 04 | The Web Frontend | scaffold | `chapter-04` | Leptos components (Button, Input, Card, Layout), Tailwind v4 theme, theoria, dokime, grocery list page, accessibility | ch03 tagged | |
| 05 | Completing the Slice | scaffold | `chapter-05` | Add-item form, server action, playwright-rust E2E + DAST, mutation testing across slice | ch04 tagged | First full ACD workflow cycle |
...
| 07 | Check Off Items | scaffold | `chapter-07` | State transitions, PATCH endpoint, optimistic UI | ch06 tagged | |
...
| 09 | Shared Grocery Lists | scaffold | `chapter-09` | Multi-user data model, authorization, real-time updates | ch08 tagged | |
| 10 | Recipe Creation | scaffold | `chapter-10` | Recipe data model, complex forms, live preview, query performance with joins | ch09 tagged | |
| 11 | Recipe-to-Grocery | scaffold | `chapter-11` | Cross-model domain logic, aggregation, dedup, unit conversion, criterion benchmarks | ch10 tagged | |
...
"Trunk to Table" riffs on "farm to table" — a concept everyone knows that means the complete, transparent pipeline from origin to plate. That's exactly what this book is about: the complete, transparent pipeline from trunk-based development to a product people use. It ties the CD philosophy (trunk) to the food domain (table) in three words. It's short, memorable, and tells you what the book is about before you open it.
...
5. **"It's all Rust" full-stack story.** Frontend, backend, API, database queries, E2E tests, schema tooling, and build tooling — all Rust. The only non-Rust pieces are Tailwind CSS for styling, Terraform HCL for infrastructure, and LinkML YAML for schema definitions.

6. **No existing book covers this combination.** Existing Rust web books (*Rust Web Development* by Gruber, *Rust Servers, Services, and Apps* by Eshwarla) cover server-side only and don't address full-stack with IaC, CI/CD, schema-driven development, or the ACD methodology.

### Scope Boundaries

**In scope for the book:**
- Full-stack Rust web application (Leptos + Axum)
- OpenAPI-compliant REST API (for mobile app consumption)
- Schema-driven development with LinkML and panschema (Rust types, SQL migrations, JSON Schema generated from a single source)
- PostgreSQL database with compile-time checked queries
...
- Native iOS and Android apps (these consume the REST API built in the book)
...
*Trunk to Table* follows the same philosophy for a different era. Where Percival taught TDD-first Python web development, this book teaches ACD-first Rust full-stack development. The reader should understand what they're signing up for: this is not a book where you skip to Chapter 8 and learn how to write a Leptos component. It's a book where you start at Chapter 1, build incrementally, and every chapter depends on the one before it. The first vertical slice — "a user can add an item to their grocery list" — takes roughly six chapters. That's not slow. That's thorough. By the end of those six chapters, the reader will have touched every layer of the stack and completed a full cycle of the ACD workflow. Every subsequent feature builds faster because the scaffolding — both in the codebase and in the reader's understanding — is already in place.
...
| Models grocery item states (pending, purchased, archived) | Enums, pattern matching, exhaustiveness checking |
...
| `panschema` | Data model definition (Chapter 2) | Schema-driven development: LinkML YAML as the single source of truth, generating Rust types, SQL DDL, and JSON Schema |
...
A grocery list and recipe management application. The domain is ideal for a teaching vehicle because:

- **Universally understood.** No time spent explaining the problem space — readers focus on engineering.
- **Not trivial.** Involves authentication, shared state (household grocery lists), relational data (recipes → ingredients → grocery items), and enough UI complexity to exercise the frontend framework.
- **Natural vertical slices.** Features decompose cleanly into independently deployable increments (add item, check off item, create recipe, generate grocery list from recipe, etc.).
- **Ruthlessly scoped MVP.** Grocery list CRUD + recipe editing/visualization. Nothing more for the book.
...
The dual-interface design creates a natural teaching progression:

1. Start with Leptos server functions to get the web app working fast (simplest path to a vertical slice).
2. In a later chapter, extract the service layer and add the REST API with utoipa.
3. Readers learn *why* the separation matters by experiencing the need for it organically.
...
- **PostgreSQL:** Production-grade database. Chosen over SQLite because the MinimumCD greenfield guide mandates production-like environments from day one. Using Linode's Managed PostgreSQL service via Terraform.
...
This provides the OpenAPI-compliant REST API that native mobile apps (and any other HTTP clients) can consume. The OpenAPI spec is generated from the same Rust types used by the Leptos frontend.
...
For MVP chapters: session-based auth with `argon2` (password hashing) and `tower-sessions` (session management). OAuth can be layered on later for the commercial product but is out of scope for the book.
...
- **`compose.yaml` with PostgreSQL:** The local development database runs in a container, matching the same PostgreSQL version as Linode's Managed PostgreSQL in staging and production. This eliminates environment divergence from day one, consistent with the MinimumCD principle of production-like environments.
- **VS Code Devcontainer (`.devcontainer/devcontainer.json`):** The recommended setup path for readers. Open the repo in VS Code, click "Reopen in Container," and get a fully configured environment: Rust toolchain, cargo-leptos, sqlx-cli, panschema, playwright-rust dependencies, and PostgreSQL as a service. No "install these 12 things before Chapter 2" section. Readers who prefer manual setup can still follow along, but the devcontainer is the happy path.
...
| Containers | Podman + compose.yaml | Local dev database, devcontainer, production images |
| Dev environment | VS Code Devcontainer | One-click setup with full toolchain + PostgreSQL |
...
- The ACD framework requires architecture to be represented as versioned delivery artifacts. For data modeling, this means the schema should be the source of truth — not the Rust code, not the SQL, not the OpenAPI spec.
...
- Introduce panschema as the tool that enforces consistency: from a single LinkML schema, generate Rust structs (with serde, sqlx, and utoipa derives), SQL DDL for migrations, and JSON Schema for contract validation.
- Contrast with the typical Rust approach: hand-maintaining separate struct layers (API DTOs, domain models, DB row types) connected by `From`/`Into` impls. That works, but consistency depends on developer discipline. Schema-driven development makes the pipeline enforce it.
...
Quality Gates:
- [ ] Test architecture established (unit with cargo-nextest, integration with SQLx test fixtures, E2E with playwright-rust)
...
**The reader ends with:** A URL they can hit (over HTTPS) that returns "ok," deployed through the full pipeline. The LinkML schema is versioned in the repo, panschema is integrated into the build, `cargo-mutants --in-diff` runs as a pipeline gate, a nightly full mutation sweep is configured, Dependabot and GitHub security scanning are active, and every MinimumCD pipeline basic is in place.

### Chapter 3: The Database

**The reader builds:** A managed PostgreSQL database on Linode (via Terraform), the `grocery_items` table (from the SQL DDL generated by panschema in Chapter 2), a service layer function to insert an item using the generated Rust types, and the first unit test.
...
- The service layer pattern: domain logic separated from any web framework, testable in isolation. Service functions operate on the generated domain types.
- `Result<T, E>` and the `?` operator in practice — writing fallible functions that propagate errors cleanly.
- Production error handling with `thiserror`: defining domain-specific error types (`ServiceError`, `DatabaseError`), adding error context and chains, mapping errors to appropriate HTTP status codes. The difference between errors for developers (logs) and errors for users (responses).
- `cargo-nextest`: how it differs from `cargo test`, how to run specific tests, how to read output.
- Writing tests against a real database: test fixtures, transaction rollback, database isolation.
...
**The reader ends with:** A passing test in CI that proves they can write to and read from a production-like database, using types and migrations that trace back to the LinkML schema. The service layer has domain-specific error types with context. The mutation testing pipeline gate (`--in-diff`) catches any untested behavior in the change. No UI yet.
...
**The reader ends with:** A themed web page with a component library, deployed to production, that displays grocery items fetched from the database. The component explorer is accessible at a dev route. Read-only for now, no way to add items yet.

### Chapter 5: Completing the Slice

**The reader builds:** A form to add grocery items, form submission handling, persistence, and the first E2E test with playwright-rust.
...
- **playwright-rust in depth:** The architecture (Rust API → JSON-RPC over stdio → Playwright server → browsers). Installing browsers in CI. Writing a meaningful E2E test: load the page, type an item name, click add, verify it appears. Assertions, waiting strategies, selectors.
- How E2E tests fit into the pipeline: the testing stage in GitHub Actions, headless browser execution, failure reporting.
- **Security-focused E2E tests (DAST):** Writing playwright-rust tests that probe the running application for vulnerabilities: attempting SQL injection via form inputs, testing for XSS in rendered output, verifying that unauthenticated requests are rejected. These run against the staging deployment as a pipeline stage.
...
- Mutation testing across the full slice: running `cargo mutants` against the complete vertical slice (service layer, server functions, form handling). The reader sees how different test layers (unit, integration, E2E) kill different categories of mutants. Surviving mutants guide where to strengthen tests.

**The reader ends with:** A *deployed, tested, working feature*. A user can add an item to their grocery list via a web browser, and both functional and security E2E tests prove it works. Mutation testing confirms the test suite catches real regressions across the full stack. The first complete trip through the ACD workflow.
...
**The reader learns:**
- Why the dual-interface architecture matters — the reader has been using Leptos server functions, and now they see why that's not enough for mobile apps.
...
- **CORS policy:** Configuring Cross-Origin Resource Sharing for the REST API. Which origins are allowed, why this matters for mobile apps and browser security.
- **Input validation at the API boundary:** Validating request payloads at the handler level before they reach the service layer. Panschema-generated types enforce shape; explicit validation enforces business rules (string length, allowed characters, etc.).
...
**The reader ends with:** The same "add a grocery item" feature accessible via both the web UI (Leptos) and a REST endpoint (documented with Swagger UI), with CORS configured, input validation enforced, list endpoints paginated, and caching headers in place. The dual-interface architecture is real and working.

### Chapters 7+: Building on the Foundation

With the full stack and ACD workflow in place, subsequent features move faster. Each introduces new concepts as needed, but the pipeline, architecture, and testing patterns are established. Planned features (each one or two chapters):

- **Check off / uncheck items** — Teaches state transitions, PATCH endpoints, optimistic UI updates. First concrete use of API versioning if the PATCH shape differs from initial design.
- **User authentication** — Teaches `argon2` password hashing, `tower-sessions`, middleware, protecting routes. Introduces secrets lifecycle: credential storage, rotation without downtime, different secrets per environment.
...
tags:
  ...
  chapter-03      ← end state after Chapter 3 (database + service layer + first test)
  chapter-04      ← end state after Chapter 4 (Leptos frontend, read-only)
  chapter-05      ← end state after Chapter 5 (add item form + E2E test)
...
This codebase is the companion to [Trunk to Table](link).
...
├── .devcontainer/
│   └── devcontainer.json         # VS Code devcontainer (full toolchain + PostgreSQL)
├── compose.yaml                  # Podman/Docker Compose (PostgreSQL for local dev)
├── Containerfile                 # Production image (multi-stage, hardened in ch7+)
├── .github/
│   └── workflows/
...
├── t2t-book-plan.md             # This planning document
└── CLAUDE.md                    # Project context for AI agents
```
...
5. The submodule pointer is updated.

This approach ensures each chapter shows the code as it existed at that point in the book's progression, not the latest version.
